...
LEGACY_STATIC_DIR = os.path.join(APP_ROOT, "legacy_static")
DB_PATH = os.environ.get("DB_PATH", os.path.join(APP_ROOT, "interviews.db"))
...
app = FastAPI(title="Digital Learning Autobiography Interview (Legacy MVP)", version="2.0", lifespan=lifespan)
app.mount("/static", StaticFiles(directory=LEGACY_STATIC_DIR), name="static")


@app.get("/")
async def read_index():
    return FileResponse(os.path.join(LEGACY_STATIC_DIR, "index.html"))
